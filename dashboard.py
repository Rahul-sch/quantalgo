...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("📊 QQQ Algo Trading Engine")
    st.caption(f"Engine: {'✅ loaded' if HAS_ENGINE else '❌ not loaded'}")
    st.caption(f"yfinance: {'✅' if HAS_YFINANCE else '❌'}")
...
st.title("📈 QQQ Algo Trading Dashboard")
...
with col1:
    ...
    if "error" in vix_data:
        st.metric(
            label="🛡️ VIX Shield",
            value="Unavailable",
            delta=vix_data["error"],
        )
        st.caption("⚠️ Data unavailable — check connection")
    else:
        vix_val = vix_data["current"]
        vix_delta = vix_data["delta"]
        status_label = "🟢 TRADING ACTIVE" if vix_val < 25 else "🔴 BLOCKED"
        bg_color = "#1a2e1a" if vix_val < 25 else "#2e1a1a"
        delta_str = f"{vix_delta:+.2f} from yesterday"

        st.markdown(
            f"""
            <div style="background:{bg_color};padding:12px;border-radius:8px;text-align:center">
                <div style="font-size:0.85em;color:#aaa">🛡️ VIX Shield</div>
                <div style="font-size:2em;font-weight:bold;color:#fff">VIX: {vix_val:.1f}</div>
                <div style="font-size:0.8em;color:#bbb">{delta_str}</div>
                <div style="font-size:1em;margin-top:6px">{status_label}</div>
            </div>
            """,
...
with col2:
    ...
    if "error" in macro_data:
        st.markdown(
            """
            <div style="background:#1e2329;padding:12px;border-radius:8px;text-align:center">
                <div style="font-size:0.85em;color:#aaa">📈 Macro Trend</div>
                <div style="color:#ef5350">Data unavailable</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        bg_color = "#1a2e1a" if above else "#2e1a1a"
        sign = "+" if pct >= 0 else ""
        label = "🟢 BULLISH" if above else "🔴 BEARISH"
        direction_str = "above" if above else "below"

        st.markdown(
            f"""
            <div style="background:{bg_color};padding:12px;border-radius:8px;text-align:center">
                <div style="font-size:0.85em;color:#aaa">📈 Macro Trend (20M SMA)</div>
                <div style="font-size:1.6em;font-weight:bold;color:#fff">{sign}{pct:.1f}%</div>
                <div style="font-size:0.8em;color:#bbb">{direction_str} 20-Month SMA</div>
                <div style="font-size:1em;margin-top:6px">{label}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )

# Card 3 — Next Blackout
with col3:
    ...
    if not blackout_windows:
        st.markdown(
            """
            <div style="background:#1e2329;padding:12px;border-radius:8px;text-align:center">
                <div style="font-size:0.85em;color:#aaa">🚫 Next Blackout</div>
                <div style="font-size:0.9em;color:#aaa;margin-top:8px">
                    Calendar not loaded<br>run paper_trader.py first
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        st.markdown(
            ...
            <div style="background:{color};padding:12px;border-radius:8px;text-align:center">
                <div style="font-size:0.85em;color:#aaa">🚫 Next Blackout</div>
                <div style="font-size:1.1em;font-weight:bold;color:#fff;margin-top:8px">
                    {content}
                </div>
            </div>
            """,
...
st.caption(
    "QQQ Algo Dashboard · "
    f"Engine: {'active' if HAS_ENGINE else 'disabled'} · "
    f"Data: {len(df_full):,} bars loaded · "
    f"Updated: {now_str}"
)
